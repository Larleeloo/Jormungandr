...
> Asset reference for the Jormungandr roguelike game.
> All paths are relative to `app/src/main/assets/`.
...
```
assets/
├── backgrounds/                  # Background images (FLAT — no subfolders)
│   └── {biome}_bg{1-3}.png      # e.g. hub_bg1.png, forest_bg2.png
├── data/
│   ├── creatures.json            # All creature definitions (45 creatures)
│   └── items.json                # All item definitions (277 items)
├── entities/
│   └── hostile/                  # Creature sprites
│       └── {creature_id}.png     # e.g. dungeon_rat.png
├── items/
│   ├── accessory/                # Rings, necklaces, bracelets, orbs
│   ├── ammo/                     # Arrows, bolts, cannon balls
│   ├── armor/                    # Chestplates, leggings, gauntlets
│   ├── clothing/                 # Shirts, dresses, hats, shoes, robes
│   ├── dye/                      # Color dyes
│   ├── food/                     # Consumable food items
│   ├── key/                      # Keys for locked doors
│   ├── material/                 # Crafting materials, ores, gems
│   ├── misc/                     # Miscellaneous collectibles
│   ├── potion/                   # Health, mana, stamina potions
│   ├── scroll/                   # Magic scrolls
│   ├── shield/                   # Shields
│   ├── tool/                     # Pickaxes, shovels, fishing rods
│   └── weapon/                   # Swords, bows, staffs, axes
└── sounds/
...
- [Asset Loading Reference](#asset-loading-reference)
...
Folder: `backgrounds/` (flat — files go directly here, no subfolders)

Loaded by `RoomCanvasView.loadAsset()` as `backgrounds/{backgroundId}.png` or `.gif`.
Background IDs are generated by `RoomGenerator.generateBackgroundId()` using the pattern `{BIOME}_bg{1-3}`.

| Region | Biome | Background IDs | Example Path |
|--------|-------|---------------|--------------|
| 0 | Desert Marketplace (Hub) | `hub_bg1` | `backgrounds/hub_bg1.png` |
| 1 | Red Dungeon | `red_dungeon_bg{1-3}` | `backgrounds/red_dungeon_bg1.png` |
| 2 | Volcanic Waste | `volcanic_waste_bg{1-3}` | `backgrounds/volcanic_waste_bg1.png` |
| 3 | Meadow | `meadow_bg{1-3}` | `backgrounds/meadow_bg1.png` |
| 4 | Forest | `forest_bg{1-3}` | `backgrounds/forest_bg1.png` |
| 5 | Ocean | `ocean_bg{1-3}` | `backgrounds/ocean_bg1.png` |
| 6 | Castle | `castle_bg{1-3}` | `backgrounds/castle_bg1.png` |
| 7 | Ice Cave | `ice_cave_bg{1-3}` | `backgrounds/ice_cave_bg1.png` |
| 8 | The Void | `void_bg{1-3}` | `backgrounds/void_bg1.png` |

---

## Creatures (Hostile Entities)

Folder: `entities/hostile/`

Sprite path is defined per creature in `data/creatures.json` as `spritePath`.
...
All item sprites are loaded from `items/{category}/` as defined by each item's `spritePath` in `data/items.json`.
...
| **Dyed Dresses** | `white_dress` thru `black_dress` | `items/clothing/{color}_dress.png` |
| **Dyed Hats** | `white_hat` thru `black_hat` | `items/clothing/{color}_hat.png` |
| **Dyed Pants** | `white_pants` thru `black_pants` | `items/clothing/{color}_pants.png` |
| **Dyed Robes** | `white_robe` thru `black_robe` | `items/clothing/{color}_robe.png` |
| **Dyed Shoes** | `white_shoes` thru `black_shoes` | `items/clothing/{color}_shoes.png` |
...
Folder: `sounds/music/`
Loaded by `SoundManager.playMusic(musicName)` via `sounds/music/{musicName}`.
...
Folder: `sounds/sfx/`
Loaded by `SoundManager.playSfx(sfxName)` via `sounds/sfx/{sfxName}`.
...
| Manager Class | Loads From | Method | Code Location |
|--------------|-----------|--------|---------------|
| `RoomCanvasView` | `backgrounds/{id}.png` or `.gif` | `loadAsset()` | `RoomCanvasView.java:439` |
| `GameAssetManager` | Any `assets/` path | `loadSprite(path)`, `loadSpriteById(id, category)` | `GameAssetManager.java:43,65` |
| `SpriteLoader` | Any `assets/` path | `loadScaled(ctx, path, w, h)` | `SpriteLoader.java:18` |
| `SoundManager` | `sounds/music/`, `sounds/sfx/` | `playMusic(name)`, `playSfx(name)` | `SoundManager.java:48,83` |
| `ItemRegistry` | `data/items.json` | `getItem(itemId)` | `ItemRegistry.java:25` |
| `CreatureRegistry` | `data/creatures.json` | `getCreature(creatureId)` | `CreatureRegistry.java:24` |

> **Note:** Directories currently contain only `.gitkeep` placeholder files.
